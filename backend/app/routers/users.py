...
def list_users(db: Session = Depends(get_db)) -> list[UserRead]:
    ...
    payload: list[UserRead] = []
    for user in users:
        # Sanitize invalid email domains (reserved TLDs like .local, .test, etc.)
        email = user.email
        if email:
            domain = email.split("@")[-1].lower() if "@" in email else ""
            # Check for reserved/special-use TLDs that Pydantic rejects
...
@router.post("/{user_id}/password", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_admin)])
...
@router.delete("/{user_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_roles("approver", "manager"))])
def deactivate_user(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.post("/{user_id}/reactivate", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_roles("approver", "manager"))])
def reactivate_user(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
@router.delete("/{user_id}/hard", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_admin)])
...
def read_me(current_user: User = Depends(get_current_user)) -> UserRead:
    ...
    if email:
        ...
        reserved_tlds = {"local", "test", "example", "localhost", "invalid"}
        if domain.split(".")[-1] in reserved_tlds:
            email = f"user{current_user.id}@example.com"
    return UserRead(
        id=current_user.id,
